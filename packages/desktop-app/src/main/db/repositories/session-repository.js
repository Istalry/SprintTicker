...
export class SessionRepository {
     ...
     */
    getActiveSession() {
        const stmt = this.dbConn.getDb().prepare('SELECT * FROM active_sessions WHERE status IN (\'TRACKING\', \'PAUSED\') ORDER BY start_time_utc DESC LIMIT 1');
        const row = stmt.get();
        if (!row)
            return null;
     ...
     */
    updateStatus(sessionId, status, totalPausedSeconds, lastPauseStartUtc) {
        const stmt = this.dbConn.getDb().prepare(`
     ...
     */
    recordPauseInterval(sessionId, pausedAtUtc, resumedAtUtc) {
        const stmt = this.dbConn.getDb().prepare(`
