...
export class DatabaseService {
     ...
     */
    getActiveSession() {
        const stmt = this.db.prepare('SELECT * FROM active_sessions WHERE status IN (\'TRACKING\', \'PAUSED\') LIMIT 1');
        const row = stmt.get();
        if (!row)
            return null;
    ...
    `);
        stmt.run(session.sessionId, session.projectId, session.taskId, session.taskKey, session.taskTitle, session.isAdHoc ? 1 : 0, session.startTimeUtc, session.status, session.totalPausedSeconds, session.lastPauseStartUtc || null);
    }
    /**
     * Updates an existing session's status and paused duration.
     */
    updateSessionStatus(sessionId, status, totalPausedSeconds, lastPauseStartUtc) {
        const stmt = this.db.prepare(`
     ...
     */
    getPendingWorklogs() {
        const stmt = this.db.prepare('SELECT * FROM worklog_sync_queue WHERE status = \'PENDING\'');
        return stmt.all();
    }
    /**
